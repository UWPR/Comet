...
AScoreOutput AScoreCalculator::Run(Peptide& peptide, const Scan& inputScan)
{
   ...
   targetMod.setResidues("ST");  // options_.getResidues());

printf("\nOK running CalculateScore with peptide %s\n", peptide.toString().c_str());
printf("OK targetMod.symbol: %c, residue: %s\n", targetMod.getSymbol(), targetMod.getResidues().c_str());
   ...
   for (const auto& mod : options_.getDiffMods())
   {
      ...
      {
printf("OK diffmod %lf, symbol %c\n", mod.getMass(), mod.getSymbol());
   ...
   {
printf("OK mod.symbol: %c, residue: %s, mass: %.6f\n", mod.getSymbol(), mod.getResidues().c_str(), mod.getMass());
      if (mod.getSymbol() == targetMod.getSymbol())
      {
         ++output.modCount_;
      }
