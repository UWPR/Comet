...
```bash
cd /mnt/c/Work/Comet-master/RealtimeSearch/bin/x64/Release
./RealtimeSearch.exe [query.raw] [MS1reference.raw] [database.idx] [num_threads] [ascorepro] [index_search_type]
```
...
`ascorepro` (default 1): 0=off, 1=localize all variable mods.
`index_search_type` (default 1): 0=PI_DB, 1=FI_DB.
Per-PSM results are written to `rts.out` in the current directory; the same
summary stats below are printed to both `rts.out` and the console.
