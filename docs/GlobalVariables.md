...
| `g_fragmentPeptides` | `VariantArray` (SoA, core/Types.h) | Mass-sorted list of all (peptide, mod-state) combinations, 13B/entry with a fixed-point mass key (docs/20260827_PI_memory.md Section 7.1). Each entry references a row in `g_vRawPeptides` via `vuiWhichPeptide`; exact masses are recomputed per candidate via `CometFragmentIndex::ComputeIndexedPepMass()`. With `decoy_search != 0` (per the `.idx` header), one pseudo-reverse internal-decoy twin per target variant, flagged by bit 7 of `vucFlags` (`VariantArray::DECOY_FLAG` / `IsDecoy()`); the twin shares the raw-peptide row, mod entry, terminal mods and mass key and only differs in which posting-list bins reference it -- `SearchFragmentIndex()` reverses sequence + mod sites on the fly for flagged candidates (docs/20260914_FI_internal_decoys.md). |
...
| `PEPTIDE_MOD_SEQ_IDXS` | `int*` -- maps peptides to their modifiable sequence index; -1 = no modifiable sequence (with terminal-mod search every peptide has one, since the two sentinel positions are always present). |
| `g_iTermSlotBytes` | `int` -- 2 when `PermuteIndexPeptideMods()` ran the permuter with terminal positions (`bVarTermModSearch`), else 0. Every `MOD_NUMBERS_POOL` entry then starts with the N-/C-term slot bytes; see `ModEntryTermSlot()` / `ModEntryResidueOffset()` (core/Types.h) and docs/20260915_permuter_terminal_mods.md. |
