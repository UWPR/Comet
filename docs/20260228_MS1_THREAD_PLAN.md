...
|--------|-----------------|-------|
| `g_vSpecLib` | [x] | Populated once by `LoadSpecLibMS1Raw()`, read-only during search |
| `g_staticParams` | [x] | Set once during initialization |
| `g_bSpecLibRead` | [x] | Written once during single-threaded init; .NET Task scheduling provides happens-before |
| `g_pvQueryMS1` | [x] (batch only) | No longer used by single-spectrum path; only batch `RunMS1Search(ThreadPool*)` |
| `dMaxSpecLibRT` | [x] | Written once during single-threaded init, read-only after |
...
- **Status:** [x] Complete (audit passed -- no mutations after init)
...
- **Status:** [x] Complete (no code change needed beyond defensive guard)
