...
note).
**2026-08-27 update -- driver port and consolidation:** every bash/awk pipeline driver named
in this document was ported to stdlib-only Python and consolidated under one umbrella CLI,
`tools/carafe.py`, so the whole ahead-of-time pipeline now runs identically under WSL
Ubuntu and a native Windows terminal. The old names appearing in the historical narrative
below map as: `carafe_prerun.sh` -> `carafe.py prerun` (`tools/carafe_prerun.py`);
`run_carafe_chunked.sh` -> `carafe.py predict` (`tools/run_carafe_chunked.py`);
`build_carafe_mask_chunked.sh` + `split_variant_map_for_chunks.awk` -> `carafe.py
mask-chunks` (`tools/build_carafe_mask_chunked.py`; the awk splitter now lives in
`carafe_chunk_common.split_variant_map()`); `params_to_fi_mask.sh` -> `carafe.py prerun
--params comet.params` (folded into the prerun driver itself). The `.sh`/`.awk` files are
deleted. Stage names, `.done` markers, and output layouts were preserved exactly, so any
workdir referenced below resumes unchanged under the Python drivers; the chunk and
variant-map splitters were verified byte-identical to the bash/awk originals. Repo-wide,
`.py` files switched from CRLF to LF line endings as part of this
(`docs/CometCodingStyleGuidelines.md` Section D). Section 3's per-tool entries below are
updated to the current names; Sections 2 and 4-11 keep the historical names, readable
...
### 3.2 `tools/run_carafe_chunked.py` (`carafe.py predict`) -- chunked, resumable Carafe inference driver

**Language**: Python 3 (stdlib; originally Bash, ported 2026-08-27). **Purpose**: `ai_pred.py` reads its entire `--in_file` into one pandas
...
### 3.7 Legacy / superseded: `tools/build_carafe_mask_chunked.py` (`carafe.py mask-chunks`) + `tools/merge_carafe_fi_masks.py` (`carafe.py merge-masks`)

**Language**: Python 3 (stdlib; the driver half was originally Bash + an awk variant-map
splitter, ported/absorbed 2026-08-27 -- see `carafe_chunk_common.split_variant_map()`). The chunked-TSV mask-build path that preceded the
...
score would be the right tool to adjudicate these, not raw xcorr; Section 6.5.1 below traces
one such scan end-to-end and identifies the exact mechanism), and a smaller handful (4
...
### 6.5.1 Case study: why a shifted phospho-site call happens (scan-level root cause)

Scoring and modification assignment in Comet are fully deterministic, so "same backbone,
different phospho site" between two deterministic searches deserves a mechanistic
explanation, not a shrug. One of Section 6.5's shifted-site scans was traced end-to-end
(analysis run 2026-08-29, Comet `2026.02 rev. 3` post-memory-optimization-merge, which
reproduces the original RTS calls for this scan exactly): **R1 scan 19549**, backbone
`RCLSYGGTSQLR` (z3, `sp|O95382|M3K6_HUMAN`), phospho on S4 vs. Y5:

| | reported site | xcorr |
|---|---|---|
| unmasked | Y5 -- `K.RCLSY[79.9663]GGTSQLR.V` | 1.4580 |
| masked | S4 -- `K.RCLS[79.9663]YGGTSQLR.V` | 1.4580 |

Method: both runs re-executed as single-scan batch searches (`scan_range = 19549 19549`,
`num_output_lines = 10`, everything else identical to `q_nonl_masked_fromcps.params`)
against the same `phospho_noNL.idx` and `phospho_charge2_noNL_fromcps.fi_mask` the RTS runs
used (the masked repro's FI entry count, 1.161e9, matches the original run's console log
exactly; unmasked is 3.540e9). Three facts compose into the answer:

1. **The spectrum cannot distinguish the two isomers.** For S4-vs-Y5 phospho on this
   sequence the theoretical spectra differ at exactly two ladder ions: b4 (`RCLS`, contains
   S4 but not Y5) and y8 (`YGGTSQLR`, contains Y5 but not S4); every other b/y ion contains
   both sites or neither and is identical between the isomers. Scan 19549's peak list (118
   peaks, extracted from `MM2_R1.mzXML`) contains **none** of the eight discriminating m/z
   values (b4 and y8, each in both phospho states, at 1+ and 2+). Every ion either isomer
   matches is a shared ion, so XCorr is bit-identical (1.4580; both isomers report
   `ions_matched = 10`). The site assignment is intrinsically ambiguous from this spectrum;
   xcorr has no opinion.

2. **Unmasked: both isomers are candidates, and a deterministic tie-break picks Y5.** The
   unmasked top-10 stores both isomers at 1.4580 (then the T8 isomer at 0.822). Equal-xcorr
   ordering is decided by the explicit tie-break in `CometSearch.cpp`'s store logic (equal
   score -> lower sequence -> "lower mod state", i.e. prefer the variable mod *later* in the
   sequence, via the `piVarModSites` lexicographic compare) -- which puts Y5 first. That
   rule imposes a total order, so within one search the winner never depends on thread
   scheduling or candidate arrival order.

3. **Masked: the Y5 variant never becomes a scoring candidate at all.** The mask is
   per-*variant* -- Carafe predicts fragment intensities separately for each positional
   isomer, so different fragment subsets survive into the FI for S4 vs. Y5. In the masked
   top-10 the Y5 isomer is absent entirely (not demoted -- absent). Rerunning the masked
   search with `fragindex_min_ions_score = 1` (production value: 3) brings it back at the
   same 1.4580, winning the tie again. So at the production threshold the Y5 variant's
   mask-surviving index fragments intersected this spectrum's top-150 peaks in only 1-2
   ions -- below the >= 3-matched-index-ions requirement to be scored -- while the S4
   variant retained >= 3. S4 then wins by walkover, at the identical score.

**Conclusion.** Determinism is not violated: each search is deterministic and repeatable.
What differs is the **candidate pool** -- which is the entire point of the mask. For scans
whose spectra contain no site-determining ions, the positional isomers tie exactly, and the
reported site is decided by candidacy (and, when both are candidates, the mod-state
tie-break) rather than by score. This also explains the shifted-site scans where the two
runs' xcorr values *differ* (e.g. R1 scan 4094: unmasked S4 at 1.950 vs. masked S7 at
1.537): XCorr for a given variant is identical in both searches, so a masked winner scoring
*lower* than the unmasked winner proves the unmasked winner was never scored in the masked
run -- the same candidate-pool mechanism, just without the tie. Section 6.5's framing
stands: these scans are genuinely site-ambiguous, and raw xcorr rank-1 is the wrong
adjudicator for them -- AScorePro's site score (or requiring observed site-determining
ions) is the right tool.
...
- `prediction/chunks/`, `prediction/chunk_preds/` -- per-chunk Carafe inference inputs/
  outputs (chunk_preds retained; pass `--delete-raw` to `carafe.py prerun` on a future run to
