...
same header lines. Given identical `comet.params` (mass range, enzyme, mods, decoy setting, etc.), an
`-i` build and a `-j` build produce a byte-identical `.idx` file -- consistent with this doc's own
Phase 0 validation ("built one unified index ... searched it as both PI_DB and FI_DB from the exact
same file").

> Superseded in part by item 4 above (Phase 0.5, same date): the mod-permutation tables and compact
> variant array this addendum originally described as on-disk content are no longer persisted at all
> -- both are regenerated from `g_vRawPeptides` + live `comet.params` once per session instead. The
> byte-identical-`.idx`-given-identical-params conclusion still holds; it just now also covers a
> smaller on-disk file (raw-peptide table + proteins list only).
...
- FI_DB additionally builds the fragment-ion posting list (`g_iFragmentIndex`/
  ...
  `g_vRawPeptides`/mod-permutation tables both modes regenerate in memory. This posting list is never
