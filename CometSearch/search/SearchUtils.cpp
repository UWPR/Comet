...
bool AllocateResultsMem(std::vector<Query*>& queries)
{
   for (std::vector<Query*>::iterator it = queries.begin(); it != queries.end(); ++it)
   {
      ...
      for (int j = 0; j < g_staticParams.options.iNumStored; ++j)
      {
         ...
         pQuery->_pResults[j].fIntensityScoreBg = 0.0;
         ...
         pQuery->_pResults[j].usiRankXcorr = 0;
         ...
         pQuery->_pResults[j].lProteinFilePosition = 0;
         pQuery->_pResults[j].lWhichProtein = -1;
         pQuery->_pResults[j].szPeptide[0] = '\0';
         pQuery->_pResults[j].cPrevAA = '-';
         pQuery->_pResults[j].cNextAA = '-';
         pQuery->_pResults[j].bClippedM = false;
         pQuery->_pResults[j].cHasVariableMod = 0;
         ...
         if (g_staticParams.options.iDecoySearch == 2)
         {
            ...
            pQuery->_pDecoys[j].fIntensityScoreBg = 0.0;
            ...
            pQuery->_pDecoys[j].usiRankXcorr = 0;
            ...
            pQuery->_pDecoys[j].lProteinFilePosition = 0;
            pQuery->_pDecoys[j].lWhichProtein = -1;
            pQuery->_pDecoys[j].szPeptide[0] = '\0';
            pQuery->_pDecoys[j].cPrevAA = '-';
            pQuery->_pDecoys[j].cNextAA = '-';
            pQuery->_pDecoys[j].bClippedM = false;
            pQuery->_pDecoys[j].cHasVariableMod = 0;
