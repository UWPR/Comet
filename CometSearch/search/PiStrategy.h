...
// Search strategy for PI_DB (plain peptide index) batch searches.
//
// initialize(): allocates search and preprocess memory pools; the peptide
//               index itself is loaded lazily by SearchPeptideIndex on first use.
// executeBatch(): LoadAndPreprocessSpectra -> RunSearch (PI path) -> PostAnalysis.
// finalize(): frees memory pools.
class PiStrategy : public ISearchStrategy
{
public:
