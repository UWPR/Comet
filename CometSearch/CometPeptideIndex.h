...
public:
   ...
   // MassType, StaticMod, VariableMod, ProteinModList, RequireVariableMod, DecoySearch,
   // Enzyme, Enzyme2 lines, terminated by the blank line separating the header from the
   // protein-name section) from an already-open file pointer. Updates g_staticParams
   // in-place, including g_staticParams.iDbType (PI_DB vs FI_DB) from IndexSearchType --
   // the .idx file is fully self-describing (docs/20260811_restore_idx_header_mods.md), no
   // index_search_type/variable_modNN/require_variable_mod/protein_modslist_file params
   // needed to search an existing index. Called by both EnsurePeptideIndexLoaded() (via
   // ReadPeptideIndex()) and InitializeMassesFromPeptideIndex() to avoid duplication, and
   // by CometSearchManager.cpp wherever it needs to peek a file's mode before deciding
