...
void CometWriteTxt::PrintTxtHeader(FILE *fpout)
{
   ...
   if (CometIntensityStore::IsEnabled())
      fprintf(fpout, "\tintensity_score\tintensity_score_bg\txcorr_pred\txcorr_pred_g\txcorr_pred_lin\txcorr_pred_n");
                                 ...
                                 const vector<Query*>& queries)  //fpdb is file pointer for either FASTA or .idx file
{
         ...
         || (iPrintTargetDecoy == 2 && ResultIsReportable(queries.at(iWhichQuery)->_pDecoys[0])))
   {
      ...
      for (int iWhichResult=0; iWhichResult<iNumPrintLines; ++iWhichResult)
      {
         ...
         fprintf(fpout, "%0.4f\t", pOutput[iWhichResult].fDeltaCn);
         if (CometIntensityStore::IsEnabled())
            fprintf(fpout, "%0.4f\t%0.4f\t%0.4f\t%0.4f\t%0.4f\t%0.4f\t", pOutput[iWhichResult].fIntensityScore, pOutput[iWhichResult].fIntensityScoreBg, pOutput[iWhichResult].fXcorrPred, pOutput[iWhichResult].fXcorrPredG, pOutput[iWhichResult].fXcorrPredLin, pOutput[iWhichResult].fXcorrPredN);
