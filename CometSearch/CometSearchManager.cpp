   ...
   if (GetParamValue("peptide_length_range", intRangeData))
   {
      if ((intRangeData.iEnd >= intRangeData.iStart) && (intRangeData.iStart > 0))
      {
         ...
         if (g_staticParams.options.peptideLengthRange.iEnd >= MAX_PEPTIDE_LEN)
            g_staticParams.options.peptideLengthRange.iEnd = MAX_PEPTIDE_LEN - 1; // as MAX_PEPTIDE_LEN is 1 larger than actual max
      }
   }
    ...
    vector<Scores>& scores)
{
   ...
   int iArraySize = (int)((g_staticParams.options.dPeptideMassHigh + g_staticParams.tolerances.dInputTolerancePlus + 2.0)
         * g_staticParams.dInverseBinWidth);
               ...
               for (int ionSeries = 0; ionSeries < NUM_ION_SERIES; ++ionSeries)
               {
                  ...
                  if (g_staticParams.variableModParameters.bUseFragmentNeutralLoss)
                  {
                     for (int iMod = 0; iMod < VMODS; ++iMod)
                     {
                        ...
                        if (isNTerm)
                        {
                           // if have not already come across n-term mod residue for variable mod iMod,
                           // see if position i contains the variable mod
