...
bool CPeptide::operator==(const CPeptide& p){
  ...
  for (i = 0; i < modification->size(); i++){
    for (j = 0; j < p.modification->size(); j++){
      if (modification->at(i) == p.modification->at(j)) break;
    }
  ...
  for (i = 0; i < substitutionModification->size(); i++){
    for (j = 0; j < p.substitutionModification->size(); j++){
      if (substitutionModification->at(i) == p.substitutionModification->at(j)) break;
    }
...
bool CPeptide::compareModsSoft(CPeptide& p){
  ...
  for (i = 0; i < modification->size(); i++){
    for (j = 0; j < p.modification->size(); j++){
      ...
      if (fabs(modification->at(i).monoisotopicMassDelta - p.modification->at(j).monoisotopicMassDelta)<0.5){
...
void CPeptide::writeOut(FILE* f, int tabs){
  ...
  if (tabs > -1){
    peptideSequence.writeOut(f,tabs+1);
    for (j = 0; j<modification->size(); j++) modification->at(j).writeOut(f, tabs + 1);
    for (j = 0; j<substitutionModification->size(); j++) substitutionModification->at(j).writeOut(f, tabs + 1);
    for (j = 0; j<cvParam->size(); j++) cvParam->at(j).writeOut(f, tabs + 1);
    for (j = 0; j<userParam->size(); j++) userParam->at(j).writeOut(f, tabs + 1);
  } else {
    peptideSequence.writeOut(f);
    for (j = 0; j<modification->size(); j++) modification->at(j).writeOut(f);
    for (j = 0; j<substitutionModification->size(); j++) substitutionModification->at(j).writeOut(f);
