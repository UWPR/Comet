...
"human.fasta" is specified by the command line option "-D".  The new .idx file
...
- Protein variable modifications are not supported in this initial FI release.
  This fuctionality is expected to be added soon. This means that variable
  modifications are limited to being applied to residues and peptide termini.
...
- For each variable_modXX, up to 5 modified residues will be analyzed. This
...
I will simply state that one can easily generate over 1 billion fragment ions in
...
- Human forward + reverse sequences (1XX,XXX sequence entries), tryptic, 1 allowed
  missed cleavage, variable mods 16M, peptide length 5 to 50
- Human forward + reverse sequences (1XX,XXX sequence entries), tryptic, 1 allowed
  missed cleavage, variable mods 16M, 80STY, peptide length 5 to 50
